...
try {
  ...
  // 5) Optionally commit and push dist when COMMIT_DIST=true
  if (process.env.COMMIT_DIST === 'true') {
    try {
      run('git add dist');
    ...
    console.log('Skipping git add/commit/push for dist (set COMMIT_DIST=true to enable).');
  }
